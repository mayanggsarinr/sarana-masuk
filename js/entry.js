...
function hitungUsia() {
  ...
  const usia = Math.max(0, Math.floor((new Date() - new Date(tglRef)) / 86400000));
  document.getElementById('calc_usia').textContent = usia + ' hari';
  if (masaEkonomis > 0)
    document.getElementById('calc_sisa').textContent = (masaEkonomis - usia) + ' hari';
}
  ...
  prosesFile('file_ba', baFiles, function () {
    prosesFile('file_foto', fotoFiles, async function () {
      await addAsset({
        ...
        nama, tglUL, tglULP, ulp,
        masaEkonomis, usia, sisa,
        realisasi: 0,
        baFiles, fotoFiles,
...
function resetForm() {
  ['f_no','f_nama','f_tgl_ul','f_tgl_ulp','f_masa_ekonomis'].forEach(id => document.getElementById(id).value = '');
