...
function bangunFilterUlp() {
  const ulps = [...new Set(semuaAssets.map(a => a.ulp).filter(Boolean))].sort();
...
function tampilData() {
  ...
  const hasil = semuaAssets.filter(a => {
    const cocokCari = !kata || a.nama.toLowerCase().includes(kata) || (a.ulp||'').toLowerCase().includes(kata);
    const cocokUlp  = filterUlp === 'semua' || a.ulp === filterUlp;
    return cocokCari && cocokUlp;
  });

  document.getElementById('infoHasil').textContent =
    (kata || filterUlp !== 'semua') ? `Menampilkan ${hasil.length} dari ${semuaAssets.length} aset` : '';
  document.getElementById('jumlahAset').textContent = semuaAssets.length + ' aset';

  const tbody = document.getElementById('tabelBody');
  if (!semuaAssets.length) {
    tbody.innerHTML = `<tr><td colspan="9" class="text-center py-5 text-muted"><i class="bi bi-inbox fs-1 d-block mb-2"></i>Belum ada data</td></tr>`;
    return;
  }
  if (!hasil.length) {
    tbody.innerHTML = `<tr><td colspan="9" class="text-center py-5 text-muted"><i class="bi bi-search fs-1 d-block mb-2"></i>Data tidak ditemukan</td></tr>`;
    return;
  }

  tbody.innerHTML = hasil.map((a, i) => {
    const foto = a.fotoFiles && a.fotoFiles.length
      ? `<img src="${a.fotoFiles[0].data}" class="file-thumb" onclick="bukaFoto('${a.fotoFiles[0].data}')">`
      : '—';
    return `<tr>
      <td class="text-muted small">${i+1}</td>
      <td class="fw-semibold" style="min-width:120px">${sorot(a.nama, kata)}</td>
      ...
      <td class="small">${a.usia}</td>
      <td class="small">${a.masaEkonomis}</td>
      <td class="small fw-bold">${a.sisa}</td>
      <td>${foto}</td>
    </tr>`;
  }).join('');
}
